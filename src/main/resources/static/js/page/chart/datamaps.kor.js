  ...
  */
  ...
  function addStyleBlock() {
    if ( d3.select('.datamaps-style-block').empty() ) {
      ...
      .html('.datamap path.datamaps-graticule { fill: none; stroke: #777; stroke-width: 0.5px; stroke-opacity: .5; pointer-events: none; } .datamap .labels {pointer-events: none;} .datamap path {stroke: #FFFFFF; stroke-width: 1px;} .datamaps-legend dt, .datamaps-legend dd { float: left; margin: 0 5px 15px 0;} .datamaps-legend dd {width: 20px;height: 20px; margin-right: 6px; border-radius: 50px;} .datamaps-legend {padding-bottom: 20px; z-index: 1001; position: absolute; right: 4px; bottom: 10px; font-size: 15px; font-family: "Helvetica Neue", Helvetica, Arial, sans-serif;} .datamaps-hoverover {display: none; font-family: "Helvetica Neue", Helvetica, Arial, sans-serif; } .hoverinfo {padding: 4px; border-radius: 1px; background-color: #FFF; box-shadow: 1px 1px 5px #CCC; font-size: 12px; border: 1px solid #CCC; } .hoverinfo hr {border:1px dotted #CCC; }');
    }
  }
  ...
  // 차트, 맵의 범례(legendary) handler
  function handleLegend(layer, data, options) {
	var { labels, radius, spacing, textOffset, bgRectWidth } = data;
    data = data || {};
    if (!this.options.fills) return;
    console.log(data, options.key);
    console.log(layer);
    // legend의 background 추가
    var g = layer.attr('transform', 'translate(50, 500)');
    var bgRect = layer.selectAll('rect').data([null]);
    console.log(bgRect);
    // 설정한 영역의 크기보다 클 경우 현재 영역의 절반 정도로 세팅
    var rectWidth = (data.bgRectWidth > this.options.element.offsetWidth/2) ? this.options.element.offsetWidth/2 : data.bgRectWidth;
    bgRect
      .enter()
      .append('rect')
      .attr('x', this.options.element.offsetWidth - rectWidth - 80)
      .attr('y', spacing * Object.keys(data.labels).length)
      .attr('rx', radius)
      .attr('width', rectWidth)
      .attr('height', spacing * Object.keys(data.labels).length + radius)
      .attr('fill', 'lightgray')
      .attr('opacity', 0.8);
      
//      bgRect.enter().append('circle').attr('r', 20);
      
      var group = d3.select(this.options.element);
      var groupEnter = group.selectAll('circle').data([null]);
...
//              .style('fill', val(datum.highlightFillColor, options.highlightFillColor, datum))
  ...
  ***************************************/
  ...
  function Datamap( options ) {
    ...
    this.addPlugin('legend', handleLegend);
