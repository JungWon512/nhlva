							...
							<c:forEach begin="1" end="${(johapData.KKO_SVC_CNT eq '' or johapData.KKO_SVC_CNT == null) ? '0' : johapData.KKO_SVC_CNT}" varStatus="st">
								<li class="video_item" style="width: 100%;height: auto;">
									<div id="player-remoteVideo${st.index }" class="player" index="${st.index }">
										<video id="remoteVideo${st.index }" style="width: 100%;background: black;min-height:230px;" poster="/static/images/assets/no_video_18980.png" muted="muted" autoplay playsinline webkit-playsinline controls></video>
									</div>
								</li>
							</c:forEach>
						</ul>
			...
			<td colspan="2">
				<div class="seeBox_gp">
					...
					<ul class="chart_label">
						<li class="gp_tit1 stand">대기 <span>${count.CNT_STAND }</span></li>
						<li class="gp_tit2 standNot">완료 <span>${count.CNT_STAND_ELSE }</span></li>
					</ul>
				</div>
			</td>
		</tr>
